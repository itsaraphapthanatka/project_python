    ...
    try:
        if csv_file_path.endswith('.xlsx'):
            ...
            for index, row in df.iterrows():
                if pd.isna(row['Image URL']):  # Update เฉพาะ row ที่ว่าง
                            ...
                            if 'Contents' in page:
                                for item in page['Contents']:
                                    ...
                                    if key.lower().endswith(('.webp')):
                                        ...
                                        if folder_name == str(listing_code):
                    ...
                    update_excel_with_links(csv_file_path, matched_links, total_rows)
                    print(f"Progress: {index + 1}/{total_rows} rows processed and updated.")
...
def update_excel_with_links(excel_file_path, matched_links, total_rows):
    try:
        df = pd.read_excel(excel_file_path)
        for index, row in df.iterrows():
            for listing_code, image_links in matched_links:
                if row['Content'] == listing_code:
                    # Combine up to 10 image links into a single string
                    image_url = ', '.join([link for links in image_links.values() for link in links[:10]])
                    df.at[index, 'Image URL'] = image_url
                # print(f"Progress: {index + 1}/{total_rows} rows updated.")
...
# if matched_links:
#     total_rows = len(matched_links) 
#     # update_excel_with_links(csv_file_path, matched_links, total_rows)
#     print(f"Progress: {total_rows} rows processed and updated.")
# else:
