        ...
        for page in paginator.paginate(Bucket=space_name):
            if 'Contents' in page:
                for item in page['Contents']:
                    key = item['Key']
                    print(f"find---{key}")
                    if key.lower().endswith(('.webp','.png', '.jpg', '.jpeg', '.gif')):  # Only process .webp files
                    # if key.lower().endswith(('.png', '.jpg', '.jpeg', '.gif')):
                        ...
                        # Set public permission for the file in DigitalOcean Spaces
                        # destination_file_path = os.path.join(destination_path, key)
                        # set_public_permission(destination_file_path)
